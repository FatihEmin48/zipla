...
(function () {
  ...
  let els = {};
  let prev = { collected: 0, stomps: 0, dead: false, won: false, jumpCount: 0, shields: 0 };
  ...
  function updateHUD() {
    els.hudLevel.textContent = `Bölüm ${world.level + 1} · ${world.name}` + (world.invincible > 0 ? ` · 🛡️${Math.ceil(world.invincible)}s` : '');
  ...
  function loop(now) {
    ...
    if (!paused) {
      ...
      if (world.shields > prev.shields) { Sound.play('win'); Render.burst(cx, cy, '#7ec8ff', 16, { speed: 180, life: 0.7, r: 3 }); }
      prev.collected = world.collected; prev.stomps = world.stomps;
      prev.dead = world.dead; prev.won = world.won; prev.jumpCount = world.jumpCount; prev.shields = world.shields;
